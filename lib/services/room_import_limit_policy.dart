...
abstract final class RoomImportLimitPolicy {
  ...
  /// 0 で自動補完オフ。429 回避のため **1〜2 件**に抑える。
  static const int postBatchAutoEnrichMaxApiCalls = 2;

  /// マイページ「情報を補完」1回あたりの API 試行上限（手動も低速キューと同じ上限）。
  static const int manualEnrichMaxApiCallsPerRun = 2;

  /// 連続する楽天商品検索 API 呼び出しの最小間隔（ミリ秒）。
  static const int enrichMinDelayMsBetweenCalls = 1350;

  /// 429 検知後、自動・手動いずれの補完も控えるクールダウン（分）。
  static const int enrichCooldownAfter429Minutes = 12;
