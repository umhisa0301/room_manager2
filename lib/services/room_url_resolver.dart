...
class RoomUrlResolver {
  ...
  static RoomUrlResolveSuccess mergeRoomResolveSuccessPreferFetched({
    ...
    String pickImg(String? listingOrCollects, String? fetchedDetail) {
      final collects = listingOrCollects?.trim() ?? '';
      final detail = fetchedDetail?.trim() ?? '';
      final apiFirst = RoomImportProductImage.pickPersistImageUrl(
        productId: 'merge',
        collectsImageUrl: collects,
        roomHtmlImageUrl: detail,
        roomPageImageUrl: detail,
      );
      return apiFirst;
    }
  ...
  static void mergeListingFastPathFromCollectsRow(
    ...
    if (row.isNotEmpty) {
      RoomImportProductImage.logCollectsResponseShape(row);
    }
    final collectsPick = RoomImportProductImage.pickProductImageFromCollectsRow(
      row,
      productId: id,
    );
    String? pageImg = collectsPick.url;
